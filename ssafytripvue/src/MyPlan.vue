    ...
    <div class="plan" v-if="showPlan">
      <div v-for="(item, index) in searchResult" :key="index" class="place-card" @click="() => toggleSelect(item)">
        ...
        <div class="place-top">
          ...
          <img class="bookmarkImg" :src="bookmarkedMap[item.name] ? '/public/RedHeart.png' : '/public/WhiteHeart.png'"
            @click="(e) => toggleBookmark(item, e)" alt="좋아요 저장" />
        </div>
      </div>
    </div>
...
const bookmarkedMap = ref({})

function toggleBookmark(item, event) {
  event.stopPropagation(); // 카드 클릭 이벤트 방지
  const key = item.name
  bookmarkedMap.value[key] = !bookmarkedMap.value[key]
}
